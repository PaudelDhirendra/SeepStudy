...
if perc<30
    cleanSigCell=cell(size(sigCell));
    rr=rr2;
    R_t=rr(:,1); %locations of R peaks in seconds
    for jj=1:length(ulab)
        fprintf('\t\tCleaning: %s\n',ulab{jj});
        sigi=find(strcmp(signalLabels,ulab{jj}));
        sig=sigCell{sigi(1)};
        [clean,nois]=ekgsuppressionMODfilt(sig,ecg,fs,fsecg,R_t,0);
        am=rms(sig);
        c=1;
        rm=rms(nois)/am;
        while (rm>0.01 && c<10)
            [cclean,nois]=ekgsuppressionMODfilt(sig,ecg,fs,fsecg,R_t,0);
            if rms(nois)/am>=rm
                rm=0;
            else
                rm=rms(nois)/am;
        ...
        for kk=1:length(sigi)
        cleanSigCell{sigi(kk)}=clean;
        end
    end
else
